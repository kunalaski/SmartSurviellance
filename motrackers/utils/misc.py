...
def draw_tracks(image, tracks, clr):
    """
    Draw on input image.
    ...
    for trk in tracks:
        ...
        # y_label = max(ymin-10, label_height)
        # cv.rectangle(image, (xmin, y_label - label_height), (xmin + label_width, y_label + baseLine),
        #                 clr[trk_id], cv.FILLED)
        # cv.putText(image, label, (xmin, y_label), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 225), 2)
        cv.putText(image, label, (xcentroid - 10, ycentroid - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, clr[trk_id], 2)
        cv.circle(image, (xcentroid, ycentroid), 4, (0, 255, 0), -1)
        cv.rectangle(image, (xmin, ymin), (xmin + width, ymin + height), clr[trk_id], 2)
